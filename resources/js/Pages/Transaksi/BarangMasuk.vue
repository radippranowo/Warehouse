...
// Debug: cek data suppliers
console.log('Suppliers loaded:', props.suppliers?.length || 0, props.suppliers);
...
function submit() {
    ...
    if (!form.supplier_id) {
        window.toast?.error('Pilih supplier');
        return;
    }
    ...
    form
        ...
        .post('/mutasi', {
            ...
            onSuccess: () => {
                ...
                router.visit('/riwayat/barang-masuk');
            },
            onError: () => {
                window.toast?.error('Gagal menyimpan pemasukan');
            },
        });
}
</script>
                ...
                <div class="card-body border">
                    <div class="d-flex align-items-center">
                        <h5 class="mb-0 card-title flex-grow-1">
                            <i class="mdi mdi-arrow-down-bold text-success me-2"></i>
                            PEMASUKAN BARANG
                        </h5>
                        <div class="flex-shrink-0">
                            <a href="/riwayat/barang-masuk" class="btn btn-secondary btn-rounded">
                    ...
                    <form @submit.prevent="submit" autocomplete="off">
                        <div class="row mb-3">
                            <div class="col-md-3">
                                ...
                                <select id="supplier_id" name="supplier_id" v-model="form.supplier_id" class="form-select" :class="{ 'is-invalid': form.errors?.supplier_id || (!form.supplier_id && form.wasSubmitted) }" required>
                                ...
                                <div v-if="form.errors?.supplier_id" class="invalid-feedback d-block">{{ form.errors.supplier_id }}</div>
                            </div>
                            <div class="col-md-3">
                                <label for="gudang_id" class="form-label">Gudang Tujuan <span class="text-danger">*</span></label>
                                <select id="gudang_id" name="gudang_id" v-model="form.gudang_id" class="form-select" :class="{ 'is-invalid': form.errors?.gudang_id }" required>
                            ...
                            <div class="col-md-2">
                                ...
                                <input id="tanggal" name="tanggal" v-model="form.tanggal" type="date" class="form-control" :class="{ 'is-invalid': form.errors?.tanggal }" required>
                            ...
                            <div class="col-md-2">
                                ...
                                <input id="referensi" name="referensi" v-model="form.referensi" type="text" class="form-control" placeholder="No. PO, Invoice">
                            </div>
                            <div class="col-md-2">
                                <label for="keterangan" class="form-label">Keterangan</label>
                                <input id="keterangan" name="keterangan" v-model="form.keterangan" type="text" class="form-control" placeholder="Keterangan">
                            </div>
                        </div>
                            ...
                            <table class="table table-bordered align-middle">
                                <thead class="table-light">
                                    <tr>
                                        ...
                                        <th style="width: 12%;">Harga Modal</th>
                                ...
                                <tbody>
                                    <tr v-for="(row, idx) in form.items" :key="row._key">
                                        ...
                                        <td style="vertical-align: top;">
                                            <input :id="`qty_${idx}`" :name="`items[${idx}][qty]`" v-model.number="row.qty" type="number" step="0.01" min="0.01"
                                        ...
                                        <td style="vertical-align: top;">
                                            <input :id="`harga_satuan_${idx}`" :name="`items[${idx}][harga_satuan]`" v-model.number="row.harga_satuan" type="number" step="0.01" min="0"
                                                class="form-control form-control-sm text-end"
                                        ...
                                        <td class="position-relative">
                                            <input :id="`keterangan_item_${idx}`" :name="`items[${idx}][keterangan]`" v-model="row.keterangan" type="text" class="form-control form-control-sm" placeholder="Opsional" :aria-label="`Keterangan barang baris ${idx + 1}`">
                                        </td>
                        ...
                        <div class="mt-3 d-flex justify-content-between">
                            ...
                            <div>
                                <a href="/riwayat/barang-masuk" class="btn btn-secondary me-2">Batal</a>
